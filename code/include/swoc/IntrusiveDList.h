...
template<typename L>
IntrusiveDList<L>::iterator::iterator(list_type *list, value_type *v) : super_type(list, v) {}
...
template<typename L>
IntrusiveDList<L>::IntrusiveDList(self_type&& that)
    : _head(that._head), _tail(that._tail), _count(that._count) {
  that.clear();
}
